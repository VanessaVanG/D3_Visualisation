...
d3.csv('combined_data.csv', function(err, combData) {
  ...
  let xLinearScale = d3.scaleLinear()
    .domain([0, d3.max(combData, d => d.insurance) + .9])
    .range([0, width]);

  let yLinearScale = d3.scaleLinear()
    .domain([0, d3.max(combData, d => d.checkup) + .6])
    .range([height, 0]);
  ...
  let circlesGroup = chartGroup.selectAll('circle')
    ...
    .enter()
    .append('circle')
    .attr('cx', d => xLinearScale(d.insurance))
    .attr('cy', d => yLinearScale(d.checkup))
    .attr('r', '15')
    .attr('fill', '#158CBA')
    .attr('opacity', '.5');
  ...
  let textLabels = circleText
    .attr('x', d => xLinearScale(d.insurance))
    .attr('y', d => (yLinearScale(d.checkup) * 1.01))
  ...
  let toolTip = d3.tip()
    ...
    .offset([80, -60])
    .html(function(d) {
      return ( `<strong>${d.state_full}</strong><br>Uninsured: 
      ${d.insurance}%<br>Checkup: 
      ${d.checkup}%`);
    });
  ...
  chartGroup.append('text')
    ...
    .attr('dy', '1em')
    .attr('class', 'axisText')
    .text('Last Medical Checkup: 5+ Years - Never (%)');
